...
LSE_PA_mean = zeros(Float64, length(Spf_angle),length(Spoofed_pmu_positions))
LSE_RMSE_abs_mean = zeros(Float64, length(Spf_angle),length(Spoofed_pmu_positions))

NLS_PA_mean = zeros(Float64, length(Spf_angle),length(Spoofed_pmu_positions))
NLS_RMSE_abs_mean = zeros(Float64, length(Spf_angle),length(Spoofed_pmu_positions))

LSE_PA_cell = Array{Vector{Float64}}(undef,length(Spf_angle),length(Spoofed_pmu_positions))
LSE_RMSE_abs_cell = Array{Vector{Float64}}(undef,length(Spf_angle),length(Spoofed_pmu_positions))

NLS_PA_cell = Array{Vector{Float64}}(undef,length(Spf_angle),length(Spoofed_pmu_positions))
NLS_RMSE_abs_cell = Array{Vector{Float64}}(undef,length(Spf_angle),length(Spoofed_pmu_positions))


for Spf_PMU in 1:length(Spoofed_pmu_positions)

    Position_Spoofed_PMUs = Spoofed_pmu_positions[Spf_PMU]
    println("Spoofed PMU:", P[Position_Spoofed_PMUs])
    
    for Spf_ang in 1:length(Spf_angle)

        Spoofing_angles = Spf_angle[Spf_ang]
        ...
        for monte in 1:monte_simulation
            ...
            LSE_PA_monte = [LSE_PA_monte; Principal_angle]
            ...
            if Principal_angle > PA_threshold
                ...
                V_k = copy(V_true)
                dig = 3
                NLS_Chi, NLS_PA, NLS_RMSE_abs, Z_correction = SRSE_algorithm(PA_threshold, Z_spf, V_pmu, tolerance, epsi, dig, P, V_k, Y, b, Fb, Tb, Nl, Nb, G, B1, B2, Flow_indices, W_inv, W_inv_sd, H_meas, H_scal,m)
                NLS_PA_monte = [NLS_PA_monte; NLS_PA]
        ...
        LSE_PA_mean[Spf_ang,Spf_PMU] = mean(LSE_PA_monte)
        LSE_RMSE_abs_mean[Spf_ang,Spf_PMU] = mean(LSE_RMSE_abs_monte)

        NLS_PA_mean[Spf_ang,Spf_PMU] = mean(NLS_PA_monte)
        NLS_RMSE_abs_mean[Spf_ang,Spf_PMU] = mean(NLS_RMSE_abs_monte)

        LSE_PA_cell[Spf_ang,Spf_PMU] = LSE_PA_monte
        LSE_RMSE_abs_cell[Spf_ang,Spf_PMU] = LSE_RMSE_abs_monte
        
        NLS_PA_cell[Spf_ang,Spf_PMU] = NLS_PA_monte
        NLS_RMSE_abs_cell[Spf_ang,Spf_PMU] = NLS_RMSE_abs_monte
...
dict_data = Dict(
    ...
    "LSE_PA_cell" => LSE_PA_cell,
    "LSE_RMSE_abs_cell" => LSE_RMSE_abs_cell,
    "NLS_PA_cell" => NLS_PA_cell,
    "NLS_RMSE_abs_cell" => NLS_RMSE_abs_cell
    )

matwrite("res_$(today_date)_Ang_var_-1to1_IEEE_$(Test_system).mat", dict_data)
