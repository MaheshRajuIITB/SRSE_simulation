...
h1 = boxplot(Chi_with_correct(:,1), 'positions', spf_ang_variation - 0.045, 'Width', 0.08,'Colors', 'b');
hold on
h2 = boxplot(Chi_with_inadequate(:,1), 'positions', spf_ang_variation + 0.045, 'Width', 0.08, 'Colors', 'r');
ylim([22 152])
yticks([40,chi_threshold_70,80,100,120,140]);
ylabel('\chi^2 Value');
l1 = yline(chi_threshold_70,'g','LineWidth',5,'DisplayName','\chi^2 threshold')
...
xlim([0.8 1.2])
xticks([1]);
xticklabels({'Non Spoofing Condition'});

resizeing_non_weights_to_weights_factor = (PA_threshold_70_with_correct_weights/PA_threshold_70_without_weights)

HA(2) = nexttile
% Create two separate boxplots with transparency
h1 = boxplot(PA_with_correct(:,1), 'positions', spf_ang_variation - 0.09, 'Width', 0.08,'Colors', 'b');
hold on
h2 = boxplot(PA_with_inadequate(:,1), 'positions', spf_ang_variation + 0.09, 'Width', 0.08, 'Colors', 'r');
hold on
h3 = boxplot(PA_without(:,1)*resizeing_non_weights_to_weights_factor, 'positions', spf_ang_variation , 'Width', 0.08, 'Colors', 'm');
ylim([0.067,0.26])
yticks([0.08,0.1,PA_threshold_70_with_correct_weights,0.16,0.2,0.24]);
