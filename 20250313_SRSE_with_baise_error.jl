...
SRSE_PA_with_correct_weights = Array{Vector{Float64}}(undef,no_of_cases,Spf_var_length)
SRSE_PA_with_inadequate_weights = Array{Vector{Float64}}(undef,no_of_cases,Spf_var_length)
SRSE_PA_without_weights =Array{Vector{Float64}}(undef,no_of_cases,Spf_var_length)

SRSE_Chi_with_correct_weights = Array{Vector{Float64}}(undef,no_of_cases,Spf_var_length)
SRSE_Chi_with_inadequate_weights = Array{Vector{Float64}}(undef,no_of_cases,Spf_var_length)
SRSE_Chi_without_weights = Array{Vector{Float64}}(undef,no_of_cases,Spf_var_length)
...
for spf_var in 1:Spf_var_length
    for nsd in 1:1
        ...
        SRSE_PA_with_correct_weights_monte = zeros(Float32, monte_simulation)
        SRSE_PA_with_inadequate_weights_monte = zeros(Float32, monte_simulation)
        SRSE_PA_without_weights_monte = zeros(Float32, monte_simulation)

        SRSE_Chi_with_correct_weights_monte  = zeros(Float32, monte_simulation)
        SRSE_Chi_with_adequate_weights_monte  = zeros(Float32, monte_simulation)
        SRSE_Chi_without_weights_monte  = zeros(Float32, monte_simulation)
        ...
        for monte in 1:monte_simulation
            ...
            for pa_te in 1:1
                ...
                if Principal_angle < PA_threshold
                    println("Principal angle final :", Principal_angle)
                    break
                else 
                    ...
                    Qms, Rms = qr(H_scal)
                    SRSE_Principal_angle = acosd((norm(inv(Rms')*(H_scal')*Z_scal_correction))/(norm(Z_scal_correction)))
                    SRSE_PA_with_correct_weights_monte[monte] = SRSE_Principal_angle
                    println("Principal angle with correct weights  after algorithm:", SRSE_Principal_angle)

                    ########^^^^^^^^^^PA test with inadequate weights ^^^^^^^^^^^######
                    Z_scal_inad_correction = W_inv_inad*Z_correction_meas
                    Qms1, Rms1 = qr(H_scal_inad)
                    SRSE_Principal_angle1 = acosd((norm(inv(Rms1')*(H_scal_inad')*Z_scal_inad_correction))/(norm(Z_scal_inad_correction)))
                    SRSE_PA_with_inadequate_weights_monte[monte] = SRSE_Principal_angle1
                    println("Principal angle with inadequate weights after algorithm:", SRSE_Principal_angle1)

                    ########^^^^^^^^^^PA without weights ^^^^^^^^^^^^^^######
                    Qms2, Rms2 = qr(H_meas)
                    SRSE_Principal_angle2 = acosd((norm(inv(Rms2')*(H_meas')*Z_correction_meas))/(norm(Z_correction_meas)))
                    SRSE_PA_without_weights_monte[monte] = SRSE_Principal_angle2
                    println("Principal angle without weights after algorithm:", SRSE_Principal_angle2)
                    

                    ##### Linear State Estimation with correct weights
                    # println("The weight matrix : \n", W_vect)
                    V_estimation1 = H_scal \ Z_scal_correction
                    Residula_vector1 = Z_correction_meas - (H_meas*V_estimation1)

                    ########^^^^^^^^^^Chi squre with correct weights^^^^^^^^^^############## 
                    SRSE_Chi_with_correct_weights_monte[monte] = abs.(((Residula_vector1')*W_inv_sd*Residula_vector1)[1,1])
                    println("Chi square with correct weights after algorithm: ", SRSE_Chi_with_correct_weights_monte[monte])


                    ##### Linear State Estimation with inadequate weights
                    # println("The inadequate weight matrix :\n", W_inad_vect)
                    V_estimation_inad1 = H_scal_inad \ Z_scal_inad_correction
                    Residula_vector_inad1 = Z_correction_meas - (H_meas*V_estimation_inad1)

                    ########^^^^^^^^^^Chi squre with inadequate weights^^^^^^^^^^^##########
                    SRSE_Chi_with_adequate_weights_monte[monte] = abs.(((Residula_vector_inad1')*W_inv_sd_inad*Residula_vector_inad1)[1,1])
                    println("Chi square with inadequate weights after algorithm: ", SRSE_Chi_with_adequate_weights_monte[monte])

                    # Linear State Estimation with inadequate weights
                    V_estimation_meas1 = H_meas \ Z_correction_meas
                    Residula_vector_meas1 = Z_correction_meas - (H_meas*V_estimation_meas1)

                    ########^^^^^^^^^^Chi squre without weights^^^^^^^^^^^^^######
                    SRSE_Chi_without_weights_monte[monte] = abs.(((Residula_vector_meas1')*Residula_vector_meas1)[1,1])
                    println("Chi square without weights after algorithm: ", SRSE_Chi_without_weights_monte[monte])


        
                    # if Principal_angle_corre <= PA_threshold || pa_te == 1
        ...
        SRSE_PA_with_correct_weights[ns,spf_var] = SRSE_PA_with_correct_weights_monte
        SRSE_PA_with_inadequate_weights[ns,spf_var] = SRSE_PA_with_inadequate_weights_monte
        SRSE_PA_without_weights[ns,spf_var] =  SRSE_PA_without_weights_monte

        SRSE_Chi_with_correct_weights[ns,spf_var] = SRSE_Chi_with_correct_weights_monte
        SRSE_Chi_with_inadequate_weights[ns,spf_var] = SRSE_Chi_with_adequate_weights_monte
        SRSE_Chi_without_weights[ns,spf_var] = SRSE_Chi_without_weights_monte
...
dict_data = Dict(
    ...
    "SRSE_PA_with_correct_weights" => SRSE_PA_with_correct_weights,
    "SRSE_PA_with_inadequate_weights" => SRSE_PA_with_inadequate_weights,
    "SRSE_PA_without_weights" =>  SRSE_PA_without_weights,

    "SRSE_Chi_with_correct_weights" => SRSE_Chi_with_correct_weights,
    "SRSE_Chi_with_inadequate_weights" => SRSE_Chi_with_inadequate_weights,
    "SRSE_Chi_without_weights" => SRSE_Chi_without_weights

    )
# matwrite("res_$(today_date)_PA_Chi_inadequate_weights_ang_vary_IEEE_$(Test_system).mat", dict_data)
matwrite("PA_Chi_inad_weights_baise_error_after_correction_alg.mat", dict_data)
