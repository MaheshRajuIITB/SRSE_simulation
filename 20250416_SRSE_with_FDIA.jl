...
for monte in 1:monte_simulation
    ...
    FDIA_meas = 1  
    FDIA_mag_percentage = 0.1
    FDIA_angle = deg2rad(5)
    Z_FDIA[FDIA_meas] = (abs(Z_FDIA[FDIA_meas]) + (FDIA_mag_percentage*abs(Z_FDIA[FDIA_meas])))*(cis(angle(Z_FDIA[FDIA_meas]) + FDIA_angle))

    FDIA_meas = 2  
    FDIA_mag_percentage = 0.5
    FDIA_angle = deg2rad(10)
    Z_FDIA[FDIA_meas] = (abs(Z_FDIA[FDIA_meas]) + (FDIA_mag_percentage*abs(Z_FDIA[FDIA_meas])))*(cis(angle(Z_FDIA[FDIA_meas]) + FDIA_angle))

    FDIA_meas = 7  
    FDIA_mag_percentage = 0.6
    FDIA_angle = deg2rad(12)
    Z_FDIA[FDIA_meas] = (abs(Z_FDIA[FDIA_meas]) + (FDIA_mag_percentage*abs(Z_FDIA[FDIA_meas])))*(cis(angle(Z_FDIA[FDIA_meas]) + FDIA_angle))

    FDIA_meas = 14  
    FDIA_mag_percentage = 0.3
    FDIA_angle = deg2rad(15)
    Z_FDIA[FDIA_meas] = (abs(Z_FDIA[FDIA_meas]) + (FDIA_mag_percentage*abs(Z_FDIA[FDIA_meas])))*(cis(angle(Z_FDIA[FDIA_meas]) + FDIA_angle))
    ...
    #######^^^^^^^^^ PA with FDIZ ^^^^^^^^^^^^###############
    Principal_angle_FDIA = acosd((norm(inv(Rm')*(H_scal')*Z_scal_FDIA))/(norm(Z_scal_FDIA)))
    # LSE_PA_monte[monte] = Principal_angle
    println("Principal angle before correction with FDIA:", Principal_angle_FDIA)
    ...
    if Principal_angle_FDIA > PA_threshold
        tolerance = 1e-3
        epsi = 0.1
        # V_k = copy(V_previous)
        V_k = copy(V_true)
